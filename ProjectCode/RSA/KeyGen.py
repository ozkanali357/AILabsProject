...
def genkeys(bits=1024): # The integer bits is 1024 which is the default key size for the key generation done with prime numbers.
    ...
    q = dobigprime(bits)
    while p == q: # It checks to be sure that p and q are not equal.
        q = dobigprime(bits)
    ...
    pubkey = (e, n)
    privkey = (d, n)
    return pubkey, privkey, p, q # It returns a tuple having the public key, the private key, the primes p and q.